  ...
  onModuleInit() {
    this.server.on('connection', (socket) => {
      ...
      socket.on('newInteractFromClient', async (data) => {
        const post_updated: Post | null =
          await this.postService.updateInteraction(data.post_id, {
            upvote: data.upvote,
        ...
        if (!isQueueToNotify && data.is_upvote) {
          setTimeout(async () => {
            let post_latest_state: Post | null =
              await this.postService.findPost(data.post_id);
            let payload_notify = {
              is_comment: false,
              post_id: data.post_id,
      ...
      socket.on('newCommentFromClient', async (data) => {
        let new_cmt: Comment = await this.commentService.createComment(
          data.user_id,
          data.cmt_cont,
          data.post_id,
          data.cmt_parent_id,
        ...
        await this.server
          .to(data.post_id)
          .emit('displayDetailCmts', payload_detail_cmts);
