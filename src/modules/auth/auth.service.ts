    ...
    try {
      ...
      const access_token = await this.jwtService.signAsync(
        payload_accesstoken,
        {
          ...
          expiresIn: parseInt(process.env.JWT_TOKEN_EXPIRY ?? '300', 10),
        },
      );
      ...
      console.log(
        `giá trị từ hardcode expiresIn access_token: ${parseInt(process.env.JWT_TOKEN_EXPIRY ?? '300', 10)}`,
      );

      return {
        access_token,
        refresh_token,
        ava_img: user.ava_img_path,
  ...
  async sendEmailReport() {
    ...
    const sendAdmin = admins.map((admin) => {
      return this.mailerService
        ...
        .then()
        .catch((err) => {
          console.error(`Lỗi khi gửi email đến ${admin.email}:`, err);
        });
    });
    await Promise.all(sendAdmin);
  }

  async sendEmailtoUserBanned(
    user_id: string,
    report_id: string,
