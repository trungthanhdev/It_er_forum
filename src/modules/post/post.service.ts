...
export class PostService {
  ...
  async changePostStatus(id: string, status: any) {
    ...
    (responsePostDetail.user_name = post?.user.user_name),
      ...
      (responsePostDetail.tags = post?.taged_bys.map((tags) => {
  ...
  async getPostAfterNSFWFiltered(postHelper: PostHelper) {
    ...
    for (const postFiltered of post) {
      const isNSFWPost = await this.containNSFW(
        postFiltered.post_content,
        this.badWordsArr,
      );
      if (!isNSFWPost) {
        ...
        (postElement.user_id = postFiltered.user.user_id),
          ...
          (postElement.is_image =
            Array.isArray(postFiltered.img_url) &&
            postFiltered.img_url.length > 0),
    ...
    resPostArr.sort(
      (a, b) => a.date_updated.getTime() - b.date_updated.getTime(),
    );
  ...
  async getPostDetailAfterNSFWFiltered(id: string) {
    ...
    (responsePostDetail.user_name = post?.user.user_name),
      ...
      (responsePostDetail.img_url = post?.img_url),
      (responsePostDetail.date_updated = post?.date_updated),
      (responsePostDetail.tags = post?.taged_bys.map(
  ...
  async searchSortPostByStatus(
    ...
    if (!Object.values(PostStatus).includes(status)) {
      throw new BadRequestException(
        `Invalid status, status must be "Pending" or "Approved" or "Rejected"!`,
      );
    }
    try {
      ...
      try {
        ...
        if (sort_by !== null) {
          response.sort((a, b) => {
            return is_ascending
              ? a.date_updated.getTime() - b.date_updated.getTime()
  ...
  async updatePost(post_id: string, post: UpdatePostDto) {
    ...
    (resUpdatePost.date_updated = postAfterUpdate.date_updated),
      ...
      (resUpdatePost.tags = postAfterUpdate.taged_bys.map(
        (tags) => tags.tag.tag_name,
      ));
  ...
  async getPostDetail(post_id: string) {
    ...
    resPostDetail.comments.sort((cmt_a, cmt_b) =>
      cmt_a.date_comment < cmt_b.date_comment ? 1 : -1,
    );
  ...
  async getPostByUserId(user_id: string) {
    const user_post = await this.postRepo.find({
      where: {
    ...
    resPostUser.sort((post_a, post_b) =>
