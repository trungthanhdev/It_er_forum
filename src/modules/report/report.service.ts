...
export class ReportService {
    ...
    try {
      if (!Object.values(ReportSubject).includes(subject)) {
        throw new BadRequestException(
          `Invalid subject, subject must be "User" or "Post" or "Comment"!`,
        );
      }
      ...
      if (sort_by !== null) {
        response.sort((a, b) => {
          ...
          return is_ascending
            ? a.date_reported.getTime() - b.date_reported.getTime()
  ...
  async sendReport(subject: ReportSubject, sendReportDto: SendReportDto) {
    const reported_user_object = await this.userService.findUserById(
      sendReportDto.reported_user_id,
    );
    ...
    switch (subject) {
      ...
      case ReportSubject.POST: {
        const postReported = await this.postService.findPost(
          sendReportDto.post_id,
        );
        if (!postReported) {
      ...
      case ReportSubject.COMMENT: {
        const commentReported = await this.commentService.findComment(
          sendReportDto.comment_id,
        );
        if (!commentReported) {
          throw new NotFoundException('Post not found!');
        }
