...
export class RecommendService {
  ...
  async getLayout(user_id: string): Promise<any> {
    const notifications: ResNotification[] =
  ...
  async getHome(user_id: string): Promise<ResHome> {
    ...
    for (let index = 0; index < subscribed_tags.length; index++) {
      ...
      let top_tag: TagedByEntity[] = await this.tagedByService.findTagById(
        element.tag.tag_id,
      );
    ...
    const recent_posts = [...posts]
      .sort((post_a, post_b) =>
        post_a.date_updated < post_b.date_updated ? 1 : -1,
      )
      .slice(0, 5);
  ...
  async getPopular() {
    ...
    trending_tags.sort((tag_a, tag_b) =>
      tag_a.num_posts < tag_b.num_posts ? 1 : -1,
    );
  ...
  async getTags(): Promise<ResTag[]> {
    ...
    for (let index = 0; index < tags.length; index++) {
      ...
      arr_num_posts[index] = await this.tagedByService.getNumberOfPost(
        element.tag_id,
      );
    }
  ...
  async getTagDetail(tag_id: string, user_id: string) {
    try {
      const chosen_tag: TagedByEntity[] =
        await this.tagedByService.findTagById(tag_id);
      const is_subscribed: boolean =
        await this.subscribedTagService.isUserSubscribeTag(user_id, tag_id);
      if (!chosen_tag) {
        ...
        if (chosen_tag.length > 0) {
          ...
          res_tag_detail.recommend_posts =
            await this.listShortPostWithCondition(chosen_tag);
        }
  ...
  private async listShortPostWithCondition(
    taged_bys: TagedByEntity[],
  ): Promise<ResPostShort[]> {
    console.log('In');
    ...
    for (let index = 0; index < taged_bys.length; index++) {
      ...
      if (!distinct_post_id_tracking.has(element.post.post_id)) {
        let res_short_post: ResPostShort =
          await this.postService.generateShortPost(element.post.post_id);
        res_tag_posts.push(res_short_post);
        distinct_post_id_tracking.set(element.post.post_id, true);
      }
    }
    res_tag_posts.sort((post_a, post_b) =>
      post_a.date_updated < post_b.date_updated ? 1 : -1,
    );
